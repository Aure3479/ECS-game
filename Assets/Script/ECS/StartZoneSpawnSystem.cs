...
public partial struct StartZoneSpawnSystem : ISystem
{
    ...
    private const int nbPillars = 36;   // piliers sur le périmètre
    private const int nbFlowers = 40;   // quantités de fleurs
        ...
        foreach (var startRef in SystemAPI.Query<RefRO<StartZonePrefabsData>>())
        {
            ...
            for (int i = 0; i < nbPillars; i++)
            {
                ...
                float3 pos = new float3(
                    math.cos(angle) * radiusPillars,
                    3f,
                    math.sin(angle) * radiusPillars
                );
